...
def mediaPerTypeExcelColuna(df, colunaType, coluna):
    tipes = df.groupby(colunaType)[coluna].mean().to_dict()
    return tipes

def contarPerTypeExcelColuna(df, colunaType, coluna):
    tipes = df.groupby(colunaType)[coluna].count().to_dict()
    return tipes

def criarDataFrame(data, colunasNames):
    df = pd.DataFrame(data, index=colunasNames).T
    return df
...
try:
    ...
    #print(criarDataFrame([
     #       somaPerTypeExcelColuna(df_vendas, "Nome da Empresa", "Vendas"),
      #      mediaPerTypeExcelColuna(df_vendas,"Nome da Empresa", "Vendas"),
       #     contarPerTypeExcelColuna(df_vendas, "Nome da Empresa", "Vendas")
        #],
        #["Total", "Média", "Contagem"]))
    print(contarPerTypeExcelColuna(df_vendas, "País de Destino", "Vendas"))
