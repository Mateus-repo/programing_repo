...
/* Exemplo prático 2: Herança com Carro e CarroEletrico ----------------------------------
.Carro será a classe base(geral)
.CarroEletrico será a classe derivada(especializada) que herda as características do carro tradicional, acrescentando o atributo autonomia da bateria.
*/
...
class cls_Carro {
protected:
    // 'protected' permite que classes derivadas acedam aos atributos diretamente (ao contrário de private)
    string atr_str_cls_Carro_Marca;
    string atr_str_cls_Carro_Modelo;
    int atr_int_cls_Carro_Ano;
...
public:
    ...
    cls_Carro(const string& atr_str_cls_Carro_marcaTemp, const string& atr_str_cls_Carro_modeloTemp, int atr_int_cls_Carro_anoTemp)
        : atr_str_cls_Carro_Marca(atr_str_cls_Carro_marcaTemp), atr_str_cls_Carro_Modelo(atr_str_cls_Carro_modeloTemp), atr_int_cls_Carro_Ano(atr_int_cls_Carro_anoTemp) {
    }

    //Métodos de acesso (getters)
    string met_str_cls_Carro_getCarroMarca() const { return atr_str_cls_Carro_Marca; }
    string met_str_cls_Carro_getCarroModelo() const { return atr_str_cls_Carro_Modelo; }
    int met_int_cls_Carro_getCarroAno() const { return atr_int_cls_Carro_Ano; }

    //Método virtual: pode ser reescrito por classes derivadas
    virtual void met_vvd_cls_Carro_mostrarInfo() const {
        cout << "Marca: " << atr_str_cls_Carro_Marca
            << ", Modelo: " << atr_str_cls_Carro_Modelo
            << ", Ano: " << atr_int_cls_Carro_Ano << endl;
    };
...
public:
    ...
    void met_vvd_cls_Carro_mostrarInfo() const override {
        // Podemos reutilizar o metodo da classe base
        cls_Carro::met_vvd_cls_Carro_mostrarInfo();
...
// Função principal
int main() {
    cls_Carro obj_cls_Carro_Carro1("Toyota", "Yaris", 2020);
    cout << "--- Carro convencional ---" << endl;
    obj_cls_Carro_Carro1.met_vvd_cls_Carro_mostrarInfo();
    /* Output:
                --- Carro convencional ---
    ...
    cout << "\n--- Carro elétrico ---" << endl;
    ce1.met_vvd_cls_Carro_mostrarInfo();
