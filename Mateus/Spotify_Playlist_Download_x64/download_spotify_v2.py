...
def print_progress(total, completed, avg_time, est_remaining, last_log):
    """Atualiza o ecrã com um resumo do progresso – chama o clear screen apenas de quando necessário."""
    os.system("cls")  # Para Windows; se utilizares Linux/Mac, substitui por "clear"
    print("=" * 60)
...
# -------------------------------------------------
# Funções de Integração – Spotify e YouTube
# -------------------------------------------------
...
def download_track_youtube(query, output_directory="dwYt"):
    ydl_opts = {
        ...
        # Se o FFmpeg não estiver no PATH, descomenta e ajusta a linha abaixo:
...
def extract_youtube_title(query):
    """Extrai o título do YouTube sem fazer download (simulate mode)."""
    ydl_opts = {
        'format': 'bestaudio/best',
        'simulate': True,
        'skip_download': True,
...
def process_track(spotify_title, query, output_directory):
    """
    Se o ficheiro já existir, extrai o título do YouTube; caso contrário, faz o download.
    Retorna (spotify_title, yt_title, elapsed, error, is_already_downloaded).
    """
    ...
    if os.path.exists(output_file):
        ...
        return (spotify_title, yt_title, elapsed, err, True)
    else:
...
if __name__ == "__main__":
    # Carrega as credenciais do Spotify e configura o ambiente
    ...
    # Carrega o URL da playlist do ficheiro url.json
    ...
    # Inverte a lista para processar do fim para o início e constrói as tarefas
    reversed_tracks = list(reversed(track_list))
    tasks = []
    for spotify_title, artists in reversed_tracks:
        query = f"{spotify_title} {artists}"
        tasks.append((spotify_title, query))
    
    total_tasks = len(tasks)
    completed = 0
    total_time_elapsed = 0.0
    update_interval = 5  # Atualiza o ecrã a cada 5 tarefas
    log_lines = []      # Acumula as linhas de log para escrita posterior

    max_workers = 8  # Número de workers para processamento concorrente
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submete todas as tarefas
        futures = {executor.submit(process_track, spotify_title, query, output_dir): (spotify_title, query) for (spotify_title, query) in tasks}
        
        for future in concurrent.futures.as_completed(futures):
            try:
                sp_title, yt_title, elapsed, err, already = future.result()
            except Exception as exc:
                # Se ocorrer um erro inesperado na tarefa, regista-o
            ...
            avg_time = total_time_elapsed / completed
            ...
            if err:
                ...
                # Caso os títulos sejam diferentes (comparação case-insensitive)
                if yt_title is None:
                    yt_title = "N/A"
            ...
            # Atualiza o ecrã apenas a cada 'update_interval' tarefas ou na última tarefa
    ...
    # Escreve todas as linhas de log de uma única vez
