...
def download_track_youtube(query, output_directory="dwYt", prefix=""):
    """
    ...
    O parâmetro 'prefix' é adicionado ao nome do arquivo se a opção music_num estiver ativa.
    """
    ydl_opts = {
        'format': 'bestaudio/best',
        'outtmpl': os.path.join(output_directory, sanitize_filename(prefix + query) + '.%(ext)s'),
        'postprocessors': [{
...
def process_track(track_position, spotify_title, query, output_directory, music_num):
    """
    ...
    O parâmetro 'music_num' define se o nome do arquivo deve ser prefixado com a posição.
    """
    prefix = f"{track_position} - " if music_num else ""
    output_file = os.path.join(output_directory, sanitize_filename(prefix + query) + ".mp3")
    if os.path.exists(output_file):
        yt_title, err = extract_youtube_title(query)
        return (spotify_title, yt_title, 0.0, err, True)
    else:
        start_time = time.time()
        yt_title, err = download_track_youtube(query, output_directory=output_directory, prefix=prefix)
...
if __name__ == "__main__":
    ...
    # Lê a opção de numeração de faixas ("music_num")
    raw_music_num = str(spotify_data.get("music_num", "")).strip()
    if raw_music_num.lower() == "true":
        music_num = True
    elif raw_music_num.lower() == "false":
        music_num = False
    else:
        print("Aviso: Valor de 'music_num' inválido ou vazio em spotify.json. Valor padrão 'False' será utilizado.")
        music_num = False
    ...
    # Cria as tarefas com a posição original de cada faixa e depois inverte a lista
    tasks = [(i, spotify_title, f"{spotify_title} {artists}") for i, (spotify_title, artists) in enumerate(track_list, start=1)]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_track, position, spotify_title, query, output_dir, music_num): (spotify_title, query)
                   for (position, spotify_title, query) in tasks}
