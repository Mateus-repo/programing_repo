...
def print_progress(total, completed, avg_time, est_remaining, last_log, error_count, warn_count):
    ...
    O clear screen é chamado apenas periodicamente para não impactar demasiadamente na performance.
    """
    os.system("cls")  # Se estiver no Linux/Mac, substitua por "clear"
    print("=" * 60)
...
def get_spotify_playlist_tracks(playlist_url):
    """Extrai o ID da playlist do URL e retorna uma lista de tuplas (título, artistas)."""
...
def download_track_youtube(query, output_directory="dwYt"):
    """
    Procura no YouTube (usando 'ytsearch1:') e baixa o áudio, convertendo-o para MP3.
...
# -------------------------------------------------
# Fluxo Principal com Contagem de Warnings e Erros e Detecção de Cancelamento via ESC
# -------------------------------------------------
if __name__ == "__main__":
    ...
    # Prepara as tarefas invertendo a lista (do fim para o início)
    ...
    completed = 0
    global_start_time = time.time()  # Tempo total de parede
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            # Verifica se alguma tecla foi premida para cancelar
            if msvcrt.kbhit():
                key = msvcrt.getch()
                if key == b'\x1b':  # ESC
                    ...
                    if answer in ['y', 's']:
                        print("Processo cancelado. A tentar cancelar as tarefas pendentes...")
            ...
            if err:
                ...
                if yt_title is None:
                    yt_title = "N/A"
                # Se os títulos não coincidirem (comparação case-insensitive), conta como warn
